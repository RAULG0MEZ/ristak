...
class DashboardMetricsService {
  ...
  async getFinancialMetrics(startDate, endDate) {
    try {
      ...
      // Get average LTV per customer
      const ltvQuery = `
        SELECT
          CASE
            WHEN COUNT(DISTINCT contact_id) > 0
            THEN COALESCE(SUM(amount), 0) / COUNT(DISTINCT contact_id)
            ELSE 0
          END as avg_ltv
        FROM payments
        WHERE paid_at >= $1 AND paid_at <= $2 AND status = 'completed'
      ...
      const [adSpend, payments, refunds, ltv] = await Promise.all([
        ...
        databasePool.query(ltvQuery, [startDate, endDate])
      ]);

      // Fetch previous period data for comparison
      const [prevAdSpend, prevPayments, prevRefunds, prevLTV] = await Promise.all([
        ...
        databasePool.query(ltvQuery, [previousStartDate, previousEndDate])
      ]);
      ...
      const avgLTV = parseFloat(ltv.rows[0].avg_ltv) || 0;

      const prevTotalSpend = parseFloat(prevAdSpend.rows[0].total_spend);
      const prevTotalIncome = parseFloat(prevPayments.rows[0].total_income);
      const prevTotalRefunds = parseFloat(prevRefunds.rows[0].total_refunds);
      const prevAvgLTV = parseFloat(prevLTV.rows[0].avg_ltv) || 0;
      ...
      const prevGrossProfit = prevTotalIncome - prevTotalSpend - prevTotalRefunds;
      const prevRoas = prevTotalSpend > 0 ? prevTotalIncome / prevTotalSpend : 0;
      const prevVatAmount = prevTotalIncome * vatRate;
      const prevNetProfit = prevGrossProfit - prevVatAmount;
      ...
      return {
        ...
        avgLTV: avgLTV,
        trends: {
          netIncome: calculateChange(totalIncome, prevTotalIncome),
          adSpend: calculateChange(totalSpend, prevTotalSpend),
          grossProfit: calculateChange(grossProfit, prevGrossProfit),
          roas: calculateChange(roas, prevRoas),
          vatToPay: calculateChange(vatAmount, prevVatAmount),
          netProfit: calculateChange(netProfit, prevNetProfit),
          refunds: calculateChange(totalRefunds, prevTotalRefunds),
          avgLTV: calculateChange(avgLTV, prevAvgLTV)
        }
      };
    } catch (error) {
